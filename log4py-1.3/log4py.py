...
Format-Parameters:
    ...
    %t -- Current time (machine readable)   #暂不支持
...
try:
    import MySQLdb   #python2鏄痬ysqldb锛岃€屽埌浜唒ython3灏卞彉鎴恗ysqlclient
...
class Logger:
    ...
    def __init__(self, useconfigfiles = TRUE, customconfigfiles = None):
        ...
        if (customconfigfiles):
            if (type(customconfigfiles) == type('1')):  #StringType
    ...
    def get_instance(self, classid = "Main", use_cache = TRUE):
        ...
        cache = Logger.cache
        #classid 参数类型：字符串，类型，实例
        #python3 引入inspect.isclass来判断
        if (isclass(classid)):  #ClassType  //class The type of user-defined classes
            classid = classid.__name__
        elif (type(classid)==type('1')):  # 字符串不变
            classid = classid
        elif (isinstance(classid,object)): #InstanceType   //instance，The type of instances of user-defined classes
        ...
        if ((classid in cache) and (use_cache == TRUE)):
            ...
            for i in range(len(targets)):
                #if (type(targets[i]) == File):  #FileType, isinstance(r, IOBase)
                if (isinstance(targets[i],IOBase) == TRUE):
                    deepcopyable = FALSE
    ...
    def __Logger_find_config(self):
        ...
        if (not self.__Logger_configfilename):
            ...
            priorities = sorted(priorities)  #todo
            configfilename = ""
    ...
    def __Logger_parse_options(self, section = SECTION_DEFAULT):
        ...
        if (configfilename != ""):
            parser = RawConfigParser()  #ConfigParser() format含%报错
    ...
    def __Logger_set_instance_options(self, parser, section, instance):
        ...
        for i in range(len(parser.options(section))):
            option = parser.options(section)[i]
            option = option.lower()
            ...
            if (option == "format"):
            	...
            	instance.set_rotation(int(value))   #rotation =1,2,3 日,月,年
            elif (option == "target"):
                splitted = value.split(",")
                instance.remove_all_targets()
                for i in range(len(splitted)):
                    instance.add_target(splitted[i].strip())
    ...
    def __Logger_cache_options(self):
        ...
        if (configfilename != ""):
            parser = RawConfigParser()  #ConfigParser()
            parser.read(configfilename)
    ...
    def __Logger_showmessage(self, message, messagesource):
        ...
        milliseconds = int(round((currenttime - int(currenttime)) * 1000))  #long(currenttime)
        ...
        #line = sub("%t", currenttime, line)    #todo
